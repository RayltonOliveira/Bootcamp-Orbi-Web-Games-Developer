...
function createLaserElement() {
  ...
  newLaser.style.top = `${yPosition - 1}px`;
  return newLaser;
}
...
function moveLaser(laser) {
  let laserInterval = setInterval(() => {
    ...
    if (xPosition === 800) {
      laser.remove();
      newLaser.classList.remove("laser");
    } else {
      laser.style.left = `${xPosition + 6}px`; // Velocidade do Tiro
    }
  }, 10);
}
...
function createAliens() {
  ...
  newAlien.style.left = "750px";
  newAlien.style.top = `${Math.floor(Math.random() * 400) + 15}px`;
